...
class EditContentLivewire extends Component
{
    ...
    public function render()
    {
        ...
        $totals=[
            'quantity' => $products->sum('orderQuantity'),
            'cost' => $products->sum('orderCost') - $this->cashbackUsed,
            'weight' => $products->sum('orderWeight'),
            'volume' => $products->sum('orderVolume'),
        ];

        $table = view('livewire.manager.orders.edit-footable-render', [
            'products' => $products,
        ])->render();

        return view('livewire.manager.orders.edit-content-livewire', [
            'table' => $table,
            'products' => $products,
            'totals' => $totals,
    ...
    public function cancelEditOrder()
    {
        if ($orig = orders()->cancelOrderEditing($this->order->order_id)){
            $this->redirectRoute('manager.orders.show', ['order' => $orig->id]);
        }
    }
    ...
    public function eventCreateOrder($payload)
    {
        ...
        try {
            ...
            $orderOriginal = orders()->updateOrderFromEdited($orderEdited->order_id, $this->bonusUsed);

            $orderOriginal->editedCopy()->delete();

            DB::commit();
            $this->redirect(route('manager.orders.show', ['order' => $orderOriginal->id]));
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    protected function revalidateProducts()
    {
        return $this->order->products
            ->each(function(Product $p){
                ...
                $p->orderWeight = $p->weight * $p->orderQuantity;
                $p->orderVolume = $p->volume * $p->orderQuantity;
                $p->isPersonalOffer = $this->isProductBelongOffer($p->pivot);
