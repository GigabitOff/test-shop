    ...
    @foreach($records as $act)
        <tr>
            ...
            <td class="text-center">
                <span
                    class="cell-price px-0 text-lowercase">{{formatMoney($act->debit)}} @lang('custom::site.uah').</span>
                <span class="cell-date">{{formatDate($act->debit_date_at)}}</span>
            </td>
            <td class="text-center">
                <span
                    class="cell-price px-0 text-lowercase">{{formatMoney($act->credit)}} @lang('custom::site.uah').</span>
            ...
            <td class="text-md-center">
                @php
                    $class = $act->isDocumentStatusSigned() ? 'success' : 'rejected';
                    $text = $act->isDocumentStatusSigned() ? __('custom::site.signed_it') : __('custom::site.not_signed_it');
                @endphp
