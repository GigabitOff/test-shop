            ...
            <table class="js-table js-table_new  footable footable-1 footable-paging footable-paging-right">
                ...
                <tr>
                    <td style="display: table-cell;">
                        <span>{{ $item->translate(session('lang')) ? $item->translate(session('lang'))->name : $item->name }}</span>
                    </td>
                    <td style="display: table-cell;">
                        <span class="title">@lang('custom::admin.products.Brand')</span><span>{{ $item->brand ? ($item->brand->translate(session('lang')) ? $item->brand->translate(session('lang'))->title : $item->brand->title) : '' }}</span>
                    </td>
                    <td style="display: table-cell;">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        @if(isset($action_product) AND count($action_product)>0)
            @this.emit('hideCategoryLivewire');

        @else
            @this.emit('hideCategoryLivewire',true);
            ...
            <div>
              ...
              <table class="js-table js-table_new footable footable-1" >
                ...
                <tbody>
                    ...
                    <td style="display: table-cell;"><a  target="_blank" href="{{ route('admin.product.edit',$item['id']) }}">{{ (!is_array($item) AND $item->translate(session('lang'))) ? $item->translate(session('lang'))->name : (isset($item['name']) ? $item['name']: '') }}</a></td>
