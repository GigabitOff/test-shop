...
<div class="search search_brand_cat {{-- --old-search--}} @if($search)is-active @endif" >
    <div class="search__controls w-100 ">
            <input type="text" id="id_search_brand_category" placeholder="@lang('custom::admin.Product categories')" wire:model.debounce.700ms="search">
    ...
    <div class="search__drop uk-drop search__drop_1 @if($search AND $search != '')is-show @endif" @if($search == '') @if(!$search__drop_show  AND isset($action_product) AND count($action_product)<5) style="display: none;"  @endif  @endif >
        <div class="search__result">
            ...
            <div class="tagger"  @if($search == '') style="display: none;" @endif>
                <input class="js-tags" type="hidden" placeholder="@lang('custom::admin.Product categories')" hidden="hidden" value="sfdsf">
                <ul>
            ...
            @if($products_entered AND count($products_entered)>0 )
                @foreach ($products_entered as $key_pr=>$item_pr)
                    <li><a href="javascript:void(0);"  class="--yellow" >
                        <span class="label">{{ isset($item_pr['name']) ? $item_pr['name'] : 'no data' }}</span>
            ...
            @endif
                    <li class="tagger-new">
                        <input class="js-tags-next" onkeypress="return addNewProducts(event,this.value)" placeholder="@lang('custom::admin.Product categories')" list="tagger-completion-1">
                      ...
                      <div class="search__result-btns">
                        ...
                        <div class="result-clear" onclick="@this.setDataProductsToDb('true');  clearNewProductsCat();">@lang('custom::admin.Cansel')</div>
 @if($products_entered AND count($products_entered)>0 )
                        <div class="result-select" onclick="@this.addProductToActionAll(); this.disabled;  clearNewProductsCat();">@lang('custom::admin.Select')</div>
                    @endif
                      </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var search_h = $('.search_brand_cat');
        $(document).click(function (e) {
        if (!$(e.target).closest(".search_brand_cat").length && search_h.hasClass('is-active')) {
         ...
         @if (isset($action_product) AND count($action_product)>0)
            //dd($this->action_product);
            @this.emit('hideProductLivewire');
        @else
            @this.emit('hideProductLivewire','true');

        @endif

        $('#id_search_brand_category').on('keypress',function(e) {
            if(e.which == 13) {
                @this.addProductToActionTmpPressEnter($('#id_search_brand_category').val());
            }
        });
    ...
    <div class="table-before-btn --catalog-table">
            <div>
              ...
              <table class="js-table js-table_new footable footable-1" >
                ...
                <th style="display: table-cell;">
                    @lang('custom::admin.Naimenuv')
                </th>

                    <th class="w-1 footable-last-visible" style="display: table-cell;" data-breakpoints="xs"></th>
                  </tr>
                </thead>
                <tbody>
