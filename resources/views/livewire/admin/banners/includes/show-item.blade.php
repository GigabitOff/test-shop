              ...
              <tr>
                ...
                <td style="display: table-cell;" class="footable-first-visible">
                    <div class="box-banners">
                        ...
                        <img width="100" src="{{ \Storage::disk('public')->url($item->image) }}" alt="@if($item->translate(session('lang'))!==null AND isset($item->translate(session('lang'))->title)){{$item->translate(session('lang'))->title}}@else{{config('app.fallback_locale') }}@endif">
                        @endif
                ...
                <td style="display: table-cell;"><a class="short-link accent"  target="_blank" href="{{ $item->link !== null ? $item->link : '' }}">{{ $item->link ? $item->link : '' }}</a></td>
                <td class="text-md-center " style="display: table-cell;">
                    @if($item->pageAndLocationText)
                        <span>{{$item->pageAndLocationText}}</span>
                    @endif
{{--                    <span>--}}

{{--                        @if(isset($item->page) AND $item->page->translate(session('lang'))!==null AND isset($item->page->translate(session('lang'))->title)){{$item->page->translate(session('lang'))->title}}@else{{config('app.fallback_locale') }}@endif--}}
